...
public class MainActivity extends AppCompatActivity {
    ...
    public boolean onStartGame(View view) {
        ...
        while(counter1 != WORD_LENGTH && counter2 != WORD_LENGTH) {
            ...
            if(choice == 1) {
                output += Character.toString(word1.charAt(counter1++));
            }
            else if(choice == 2) {
                output += Character.toString(word2.charAt(counter2++));
            }
        }
        if(counter1 != WORD_LENGTH) {
            output += word1.substring(counter1);
        }
        if(counter2 != WORD_LENGTH) {
            output += word2.substring(counter2);
        }
        for(int i = 2*WORD_LENGTH - 1; i != -1; i--) {
            LetterTile letterTile = new LetterTile(this, output.charAt(i));
            stackedLayout.push(letterTile);
        }
        messageBox.setText(word1 + " " + word2 + " " + output);
        return true;
    }
